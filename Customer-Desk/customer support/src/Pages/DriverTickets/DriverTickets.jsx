...
const DriverTickets = () => {
  ...
  // Fetch driver tickets on component mount
  useEffect(() => {
    const fetchDriverTickets = async () => {
      try {
        ...
        if (data.success) {
          const filteredDriverTickets = data.tickets
            .filter((ticket) => ticket.status === "delivery")
            .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
          setDriverTickets(filteredDriverTickets);
        }
      } catch (error) {
  ...
  // Handle real-time messaging with Socket.IO for all driver tickets
  useEffect(() => {
