    ...
    selectedTickets.forEach((ticket) => {
      socket.on(`receiveMessage_${ticket.ticketId}`, (data) => {
        setChatboxStates((prev) => {
          const updatedMessages = [...(prev[ticket.ticketId]?.messages || []), { sender: data.sender, text: data.message, time: new Date(data.timestamp).toLocaleTimeString() }];
            ...
            {pendingTickets.length > 0 ? (
              ...
              <tr>
                <td colSpan="8" style={{ textAlign: "center" }}>
                  No tickets are currently pending.
                </td>
              </tr>
            )}
          </tbody>
